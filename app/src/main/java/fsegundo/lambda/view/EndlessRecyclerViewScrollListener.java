...
/*    public EndlessRecyclerViewScrollListener(GridLayoutManager layoutManager) {
    ...
    public void onScrolled(final RecyclerView view, int dx, int dy) {
        ...
        if (mPaused || dy <= 0) return;
//        Log.i(mTag, "onScrolled-------dy:" + dy);

        RecyclerView.Adapter adapter = view.getAdapter();
        int totalItemCount = adapter.getItemCount(); //items currently available in the adapter

        int lastVisibleItemPosition = getLastVisibleItemPosition();

        // we check to see if we have breached the visibleThreshold and need to reload more data.
...
//                            Log.i(mTag, "!!!!currentpage:" + currentPage);
            ...
            if (!loading) {
                // If it isn’t currently loading, we execute onLoadMore to fetch the data.
